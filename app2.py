...
with tabs[2]:
    ...
    if eda_option == "📈 Industrial Risk vs Bankruptcy":
        ...
        fig = px.bar(
            perc_df, x='industrial_risk', y='percent', color='class_yn',
            barmode='group', text=perc_df['percent'].round(1),
            title="Industrial Risk vs Bankruptcy (%)",
            labels={'industrial_risk': 'Industrial Risk Level', 'percent': 'Percentage', 'class_yn': 'Outcome'},
        ...
        fig = px.bar(
            perc_df, x='financial_flexibility', y='percent', color='class_yn',
            barmode='group', text=perc_df['percent'].round(1),
            title="Financial Flexibility vs Bankruptcy (%)",
            labels={'financial_flexibility': 'Financial Flexibility Level', 'percent': 'Percentage', 'class_yn': 'Outcome'},
            color_discrete_map={0: 'red', 1: 'green'}
        )
        ...
        st.info(f"""
        📌 **Overall bankruptcy rate:** {bankrupt_rate:.1f}%  
        📌 Companies with **high industrial risk & bankrupt:** {high_risk_bankrupt}  
        📌 Companies with **low financial flexibility & bankrupt:** {low_flex_bankrupt}  
        📌 Companies with **high operating risk & bankrupt:** {high_operating_bankrupt}  
        """)
